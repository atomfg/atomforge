...
def _get_references(model_registration):
    ...
    if references:
        ref_str = "\n".join(f"- [blue]{ref.label}[/blue]: {ref.url}" for ref in references)
    else:
...
def _get_parameters(model_registration):
    ...
    if model_fields:
        ...
        for key, field in model_fields.items():
            ...
            type_str = field.annotation.__name__ if hasattr(field.annotation, "__name__") else str(field.annotation)
            default_str = f" (default: {field.default})" if field.default is not None else ""
            parameters_str += f"- [blue]{key}[/blue]: [green]{type_str}[/green]{default_str}\n"
