...
class UVEnvironmentProvider(EnvironmentProvider):
    ...
    def __init__(self, search_path: tuple[Path, ...], install_path: Path):
        ...
        self._core_path = Path(atomforge_core.__file__).parent.parent.parent
        self._runtime_path = Path(atomforge_runtime.__file__).parent.parent.parent
    ...
    def _install_atomforge(self, env_path: Path) -> None:
        command = [
            ...
            env_path.as_posix(),
            "-P",
            f"atomforge-core @ {self._core_path.as_posix()}",
            f"atomforge-runtime @ {self._runtime_path.as_posix()}",
        ]
