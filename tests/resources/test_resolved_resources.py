...
def test_resolved_resources_attributes():
    ...
    allowed_attributes.remove("strict") # Remove 'strict' from the allowed attributes since it's not part of ResolvedResources
    allowed_attributes.append("messages") # Add 'messages' to the allowed attributes since it's part of ResolvedResources
    ...
    for attribute in ResolvedResources.model_fields.keys():
        assert attribute in allowed_attributes, f"Unexpected attribute '{attribute}' found in ResolvedResources"

    # Check that all allowed attributes are present
    for attribute in allowed_attributes:
        assert attribute in ResolvedResources.model_fields.keys(), f"Expected attribute '{attribute}' not found in ResolvedResources"
