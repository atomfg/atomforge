...
def test_execution_resources_attributes():
    ...
    for attribute in resources.__dict__.keys():
        print(attribute)
        assert attribute in allowed_attributes, f"Unexpected attribute '{attribute}' found in ExecutionResources"

    # Check that all allowed attributes are present
    for attribute in allowed_attributes:
        assert hasattr(resources, attribute), f"Expected attribute '{attribute}' not found in ExecutionResources"
