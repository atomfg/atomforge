...
from atomforge._core.protocol.response import ShutdownResponse
from atomforge._core.protocol.request import ShutdownRequest
...
def test_prepare_model_error(backend, mocker):
    ...
    mocker.patch("atomforge._host.backend.subprocess.backend.SubprocessBackend._retrieve_environment_and_subprocess", return_value=(None, None))
    mocker.patch("atomforge._host.backend.subprocess.backend.SubprocessBackend._prepare_init_model_request", return_value=None)
    mocker.patch("atomforge._host.backend.subprocess.backend.SubprocessBackend._send_request_and_get_response", return_value=MockResponse(operation="error"))
